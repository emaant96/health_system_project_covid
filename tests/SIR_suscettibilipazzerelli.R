...
giggi <- ggplot(data = dati_reali, aes(x = tempo)) + geom_point(size = 0.8, aes(y = suscettibili, color = "Suscettibili"))
for (i in seq(1, 10, by = 1)) {
  betaRes <- lossArray[i,1]
  gammaRes <- lossArray[i,2]
  N <- NInit * lossArray[i,4]
  S0 <- N - Infetti[1] - Rimossi[1]
  ...
  giggi <- giggi + geom_line(data = mod.pred, mapping = aes(x = t,y = S, color = "Suscettibili SIR"))
}

giggi <- giggi +
  labs(title= "Confronto tra risultati Suscettibili per vari N",
